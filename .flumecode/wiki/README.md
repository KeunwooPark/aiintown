<!-- wiki-synced-to: c43cfa8ab0b542651e76f1dbde0f01d679c0da30 -->

# aiintown wiki

> **TL;DR** — `aiintown` (product name "AI in Town") is a minimal Jekyll site built and served natively by GitHub Pages whose homepage is an airport-departure-board list of cities that each link to a per-city page listing that city's upcoming AI events, plus a scheduled GitHub Actions job that uses Claude web search to collect those events into committed per-city JSON data files.

This repository is a static website, presented to users as **AI in Town** — a site that collects and shows local offline meetups across various cities. It uses [Jekyll](https://jekyllrb.com/) with its **own custom theme** — committed `_layouts/`, `_includes/`, and `_sass/` giving a stark two-ink lithograph print-shop look (ink black + vermilion accent on cream paper, flat fills, square corners, hard ink borders, and geometric primitive-shape SVG accents) with a Google Fonts type pairing (`Fraunces` display serif headlines + `Nunito Sans` body) (`minima` stays configured only as a fallback) — and is published through GitHub Pages' native build (the `github-pages` gem), deployed from the `main` branch root. There is no application backend or JavaScript build tooling. A daily GitHub Actions workflow plus a Python script searches the web (via Claude) for upcoming offline AI events per city and commits the results under `_data/events/`; at build time the homepage renders that data as a monospace airport-departure-board list of cities, and each city's events appear as cards on a dedicated per-city page.

## Navigation map

| Component   | What it does                       | Wiki page                                | Key paths                            |
| ----------- | ---------------------------------- | ---------------------------------------- | ------------------------------------ |
| Jekyll site | Departure-board city landing + per-city event pages, with client-side city search and UI language switching | [components/site.md](components/site.md) | `index.md`, `cities/`, `_layouts/`, `_includes/`, `_sass/`, `assets/css/main.scss`, `assets/js/search.js`, `_data/i18n.yml`, `_config.yml` |
| AI event search | Daily Claude search → per-city event JSON, deduped by URL | [components/event-search.md](components/event-search.md) | `_data/cities.yml`, `scripts/search_events.py`, `scripts/dedupe_events.py`, `.github/workflows/search-ai-events.yml` |
