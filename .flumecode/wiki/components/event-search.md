...
This subsystem **generates data, not pages**. A daily job collects events with an LLM and commits them into the repo as `_data/events/<city-id>.json`. Because the files live in Jekyll's `_data/` directory, the site renders them at build time via `_includes/events.html` (see [components/site.md](components/site.md)) — this subsystem stays focused on producing clean data. The search is deliberately scoped to *upcoming* events; finished events are never written.
...
- **Adjust how events render** in the Jekyll layer — `_includes/events.html` / `_includes/event-card.html` (see [components/site.md](components/site.md)).
