...
The header's **first child inside `.container`** is a `<nav class="header-links" aria-label="Repositories">` holding two plain-text brand-name anchors — `GitHub` (→ `https://github.com/KeunwooPark/aiintown`) and `FlumeCode` (→ the FlumeCode repository URL) — each `target="_blank" rel="noopener"`, with no icons. The two URLs are **hardcoded** in the include (not in `_config.yml` or `_data/`), matching the repo's minimal style for two static links. The nav lives inside `.container` deliberately: it inherits the container's `position: relative; z-index: 1`, so its absolutely-positioned styling (`_sass/_layout.scss`) pins it to the top-right and layers it above the decorative `.site-header::after` vermilion circle. Because the links render in the shared header, they appear on every page (homepage and per-city).
...
1. **Language switching** — reads `window.SITE_I18N` (injected by `_layouts/default.html` from `site.data.i18n`). `applyLang(lang)` sets `document.documentElement.lang`, then updates `textContent` for every `[data-i18n]` element and `placeholder` for every `[data-i18n-placeholder]` element using the locale dict. After swapping that chrome it also **toggles event text by locale**: it walks every `[data-city-section]` and, via `applyLocaleToSection`, computes `useLocal = section's data-city-locale === lang`. Localized spans are simple: it unhides `.t-local`/`.d-local` for matching sections and hides them otherwise. English spans use a **per-field fallback** so no card ever goes blank: the helper `hideEnglishSpanIfLocalized` hides each `.t-en`/`.d-en` **only when** `useLocal` is true *and* a paired localized sibling exists in the same parent element (`.t-en` looks for `.t-local`, `.d-en` for `.d-local`, scoped via `en.parentNode.querySelector(...)`). An event field with no localized variant therefore keeps its English text visible even on the matching locale, and title/description fall back independently. Switching back to English restores every English span and hides every localized one. This per-field pairing relies on `.t-en`/`.t-local` (and `.d-en`/`.d-local`) being siblings under the same `<a>`/`<h3>`/`.desc` parent emitted by `_includes/event-card.html`. The active locale is persisted to `localStorage` and restored on page load. The `<select id="lang-switcher">` (in `header.html`) drives the switcher.
...
- **`_layout.scss`** — `.container`, the `.site-header` hero, the `.header-links` top-right repository links, `.city-section`, the `.lang-switch` language switcher, and `.site-footer`. `.header-links` is `position: absolute` at `top/right: 1.25rem` with `z-index: 2` — measured against `.site-header .container` (which sets `position: relative; z-index: 1`), so the links sit above the decorative `::after` circle (which keeps its default stacking) and stay clickable. It is a flex row (`gap: 1rem`) in `$font-sans` weight 600; links are `$color-ink` and turn `$color-accent` on hover (the inverse of the global `a` convention in `_base.scss`). A `@media (max-width: 600px)` query drops the nav out of its absolute pin back into normal flow (`position: static`, `justify-content: flex-end`, `margin-bottom: 1rem`), so on narrow screens the header stacks as **two rows** — the right-aligned links on top, the title/tagline below — instead of the links overlapping the heading. The hero is **flat**: a solid cream `$color-bg` field with a hard ink `border-bottom`, no gradient — ink headline + vermilion tagline. Decoration is **geometric primitives** (data-URI SVGs, no raster assets): a vermilion `<circle>` in the hero corner via `.site-header::after`, and a small vermilion `<rect>` (filled square) marker before each city heading via `.city-section > h2::before`. The corner circle is a **decorative background**: because the absolutely-positioned `::after` would otherwise paint over the in-flow title/tagline, the scoped rule `.site-header .container { position: relative; z-index: 1; }` raises the header content above it (the circle keeps its default `z-index: auto`). A positive `z-index` on the content wrapper is used instead of `z-index: -1` on `::after`, which would sink the circle behind the header's opaque `background` and vanish. Section headings and the footer use the hard `$border` ink rule. The **language switcher** is themed to match the `.city-search input`: `#lang-switcher` strips native chrome (`appearance: none`) and rebuilds the closed control from the shared tokens — `$border` ink frame, square corners (`border-radius: 0`), `$color-surface` fill, `$color-ink` text, `$font-sans`, and a vermilion (`$color-accent`) chevron supplied as a data-URI SVG `background-image` (same accent-SVG approach as the markers above; `background-color` is set separately so the SVG can't clobber the cream fill). `#lang-switcher:focus` draws the `outline: 3px solid $color-accent` vermilion ring, matching `.city-search input:focus`. CSS-only — `_includes/header.html` and `assets/js/search.js` are untouched, so language switching still works.
...
- **`_departure-board.scss`** — the homepage board. It **inverts the palette** to evoke a dark split-flap departure sign while staying on-brand: a `$color-ink` panel with `$color-bg` (cream) text, square corners, and the hard `$border` ink frame, all set in `$font-mono`. `.board-head` and `.board-row` are CSS grid (`destination | country | when | gate`); the `.when` "time/status" column and the `.gate` glyph use the `$color-accent` vermilion. On **hover** a row **inverts to a full vermilion fill** (`background: $color-accent`) with `$color-ink` text — and the accent-colored `.when`/`.gate` spans are explicitly re-set to `$color-ink` too, so every span stays high-contrast (the earlier translucent `rgba($color-accent, .18)` hover left the vermilion date/arrow text washed out to near-invisible). Imported **last** in `assets/css/main.scss` (after `search`).
